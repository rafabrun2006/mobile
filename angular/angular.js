...
function $CompileProvider($provide, $$sanitizeUriProvider) {
   ...
   * @description
   * Retrieves or overrides the default regular expression that is used for whitelisting of safe
   * urls during _img[src] sanitization.
   *
   * The sanitization is a security measure aimed at prevent XSS attacks via html links.
   *
   * Any url about to be assigned to _img[src] via data-binding is first normalized and turned into
  ...
  this.$get = [
    ...
    Attributes.prototype = {
      ...
      $set: function(key, value, writeAttr, attrName) {
        ...
        // sanitize a[href] and _img[src] values
        if ((nodeName === 'a' && key === 'href') ||
            (nodeName === '_img' && key === 'src')) {
    ...
    function getTrustedContext(node, attrNormalizedName) {
      ...
      if (attrNormalizedName == "xlinkHref" ||
          ...
          (tag != "_img" && (attrNormalizedName == "src" ||
 ...
 *
 * Additionally, `a[href]` and `_img[src]` automatically sanitize their URLs and do not pass them
 ...
 * | `$sce.URL`          | For URLs that are safe to follow as links.  Currently unused (`<a href=` and `<_img src=` sanitize their urls and don't constitute an SCE context. |
 ...
 * http://stackoverflow.com/a/472729)  However, setting _img[src] does normalize the URL.
 * Unfortunately, setting _img[src] to something like "javascript:foo" on IE throws an exception.
 ...
 * ```html
 * <_img src="http://www.gravatar.com/avatar/{{hash}}"/>
 * ```
 *
 * The correct way to write it:
 * ```html
 * <_img ng-src="http://www.gravatar.com/avatar/{{hash}}"/>
 * ```
 *
 * @element IMG
 ...
 * The buggy way to write it:
 * ```html
 * <_img srcset="http://www.gravatar.com/avatar/{{hash}} 2x"/>
 * ```
 *
 * The correct way to write it:
 * ```html
 * <_img ng-srcset="http://www.gravatar.com/avatar/{{hash}} 2x"/>
 * ```
 *
 * @element IMG
